...
async function main() {
    ...
    // As deduced:
    // C# `UInt160.Parse` treats hex strings as Big Endian representing the number.
    // So the number represented by string "0x...3333" is exactly the number BigInt("0x...3333").
    // And this is the exact number expected by `EncodeUInt160Scalar`.
    
    const assetHex = "0000000000000000000000000000000000003333";
    const assetField = BigInt("0x" + assetHex);
    
    const recipientHex = "7f9acb9b3537c3b9ee4cb97a5656b595f072b665";
    const relayerHex = "7f9acb9b3537c3b9ee4cb97a5656b595f072b665";
    const recipient = BigInt("0x" + recipientHex);
    const relayer = BigInt("0x" + relayerHex);
